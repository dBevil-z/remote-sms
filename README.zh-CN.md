...
| 邮件转发可配置 | 可通过自有 SMTP 把新收到的短信转发到邮箱，发件人、收件人、主机、端口、安全方式、用户名、密码都可在运行时配置。 |
...
| 邮件发送日志可见 | SMTP 发送成功或失败会写入 App 日志，并能在网页日志页直接看到。 |
| 配置不硬编码 | frp 地址、端口、认证信息、访问密码、SMTP 配置都在运行时配置。 |
...
3. 手机接收到短信后写入本地记录。
4. 如果配置了 SMTP，新收到的短信还可以自动转发到邮箱，邮件正文保持和短信原文一致。
5. 可信浏览器访问手机的 Web 服务，通过访问密码读取短信。
6. 需要发送短信时，在网页端选择 SIM 卡、填写号码和内容。
7. 网页端可以同时查看手机电量、存储、内存、网络、SIM、发送桥等状态，也能查看邮件发送日志。
...
App 不会在代码里硬编码 frp 地址、frp token、网页访问密码或 SMTP 凭据。
...
- SMTP 主机
- SMTP 端口
- SMTP 安全方式（`ssl`、`starttls` 或 `none`）
- SMTP 用户名
- SMTP 密码或授权码
- 发件邮箱
- 收件邮箱

短信转发邮件时，邮件正文会直接使用短信原文，不做改写。邮件发送成功或失败会追加到 App 日志中，并可以在网页日志页查看。
...
- 请设置足够长的网页访问密码。
- frp 认证信息、网页访问密码、SMTP 用户名和 SMTP 密码都不要提交到 Git 仓库。
